...
export function ChatPanel({ messages, onSendMessage, username, currentSessionId }: ChatPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <h3 className="font-semibold text-gray-900 dark:text-white">{t.room.chat}</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 ? (
          <div className="text-center text-gray-500 dark:text-gray-400 py-8">
            ...
            <p className="text-sm">{t.room.startConversation}</p>
          </div>
        ) : (
          messages.map((msg) => {
            const isOwnMessage = msg.senderSessionId === currentSessionId;
            return (
              <div
                key={msg.id}
