  ...
  return (
    <div className="relative bg-gray-900 rounded-2xl overflow-hidden shadow-xl">
      <video
        ...
        autoPlay
        playsInline
        className={`w-full h-full object-cover ${!peer.isVideoEnabled ? 'hidden' : ''}`}
      />
      {!peer.isVideoEnabled && (
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-indigo-600 to-pink-600">
          <div className="w-24 h-24 rounded-full bg-white/20 flex items-center justify-center">
            <User className="w-12 h-12 text-white" />
          </div>
        </div>
      ...
      <div className="absolute bottom-4 left-4 right-4">
        <div className="bg-black/60 backdrop-blur-sm rounded-xl px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {!peer.isVideoEnabled && <VideoOff className="w-4 h-4 text-red-400" />}
          </div>
        </div>
      </div>
    </div>
